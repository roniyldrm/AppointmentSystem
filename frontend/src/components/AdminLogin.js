  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (userData.role !== 'admin') {
        setError('Bu hesap admin yetkisine sahip değil. Lütfen admin hesabınızla giriş yapın.');
        setLoading(false);
        return;
      }
      ...
      <div className="max-w-md w-full space-y-8">
        <div className="bg-white/10 backdrop-blur-md rounded-2xl shadow-2xl p-8 border border-white/20">
          <div className="text-center">
            ...
            <p className="text-red-100 text-sm">Sistem yönetimi için giriş yapın</p>
          </div>
        ...
        <div className="text-center">
          <p className="text-red-200 text-xs">
            © {new Date().getFullYear()} Hastane Randevu Sistemi - Admin Panel
          </p>
        </div>
      </div>
    </div>
